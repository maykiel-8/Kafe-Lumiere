...
<title>@yield('title', 'Dashboard') — Kafé Lumière</title>
...
<aside id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        {{-- ── CUSTOMER sidebar (only shows My Dashboard + Home) ── --}}
        @if(auth()->user()->isCustomer())
            <div class="nav-section-label">My Account</div>
            <a href="{{ route('customer.dashboard') }}" class="nav-link-item {{ request()->routeIs('customer.*') ? 'active' : '' }}">
                <i class="bi bi-grid-1x2"></i> My Dashboard
            </a>

            <div class="nav-section-label" style="margin-top:.75rem;">Account</div>
            <a href="{{ route('admin.profile.show') }}" class="nav-link-item {{ request()->routeIs('admin.profile*') ? 'active' : '' }}">
                <i class="bi bi-person-circle"></i> My Profile
            </a>

        {{-- ── ADMIN / CASHIER sidebar ── --}}
        @else
            ...
            <a href="{{ route('cashier.orders.index') }}" class="nav-link-item {{ request()->routeIs('cashier.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.transactions.index') }}" class="nav-link-item {{ request()->routeIs('admin.transactions*') ? 'active' : '' }}">
    ...
    <div class="sidebar-footer">
        ...
        <div style="flex:1;min-width:0;">
            <div style="font-size:.82rem;font-weight:500;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ auth()->user()->full_name }}</div>
            <div style="font-size:.7rem;color:rgba(255,255,255,0.5);">{{ ucfirst(auth()->user()->role) }}</div>
        </div>
...
<div id="main">
    <header class="topbar">
        ...
        <div class="d-flex align-items-center gap-3">
            <span style="font-size:.8rem;color:#888;">{{ now()->format('D, M d Y') }}</span>
            {{-- Fix: customers go back to their dashboard, staff go to POS --}}
            @if(auth()->user()->isCustomer())
                <a href="{{ route('customer.dashboard') }}" class="btn-kafe">
                    <i class="bi bi-grid-1x2"></i> My Dashboard
                </a>
            @else
                <a href="{{ route('cashier.orders.index') }}" class="btn-kafe">
    ...
    <div class="content-area">
        @if(session('success'))
            <div class="alert-kafe-success mb-3"><i class="bi bi-check-circle"></i> {{ session('success') }}</div>
        @endif
