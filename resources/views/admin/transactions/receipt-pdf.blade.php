<!DOCTYPE html>
{{-- resources/views/admin/transactions/receipt-pdf.blade.php --}}
{{-- Rendered by DomPDF — inline styles only, no flexbox/grid --}}
<html lang="en">
<head>
<meta charset="UTF-8">
<style>
...
<table class="info-table">
  ...
  <tr>
    <td>Date &amp; Time</td>
    <td>{{ $transaction->created_at->timezone('Asia/Manila')->format('F d, Y h:i A') }}</td>
  </tr>
  <tr>
    <td>Cashier</td>
    <td>{{ ($transaction->cashier && $transaction->cashier->role === 'customer') ? 'Kafé Lumière' : ($transaction->cashier->full_name ?? 'Kafé Lumière') }}</td>
  </tr>
  <tr>
    <td>Customer</td>
...
<table class="items-table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="item-meta">{{ $item->size }}</td>
      <td>{{ $item->quantity }}</td>
      <td>
        &#8369;{{ number_format($item->subtotal, 2) }}
        <div class="item-meta">&#8369;{{ number_format($item->unit_price, 2) }} each</div>
      </td>
    </tr>
    @endforeach
  </tbody>
