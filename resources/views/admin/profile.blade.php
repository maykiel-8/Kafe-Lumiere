        ...
        <div class="kafe-card text-center">
            <div class="kafe-card-body" style="padding:2rem;">
                <div id="avatarPreview" style="width:110px;height:110px;border-radius:50%;background:var(--kafe-caramel);margin:0 auto 1rem;display:flex;align-items:center;justify-content:center;font-size:2.8rem;color:#fff;font-family:var(--font-serif);overflow:hidden;cursor:pointer;" onclick="document.getElementById('photoInput').click()">
                    @if(auth()->user()->photo)
                        <img src="{{ auth()->user()->photo_url }}" style="width:100%;height:100%;object-fit:cover;" alt="Profile Photo">
                    @else
                ...
                <div style="margin-top:4px;font-size:.75rem;color:#888;">{{ auth()->user()->email }}</div>

                {{--
                    FIX: The photo-only form was missing first_name, last_name, email
                    so validation always failed. Solution: include current values as
                    hidden fields so the controller receives all required fields.
                --}}
                <form action="{{ route('admin.profile.update') }}" method="POST"
                      enctype="multipart/form-data" id="photoForm">
                    @csrf @method('PATCH')
                    {{-- Pass current values so validation passes --}}
                ...
                <div style="margin-top:1.5rem;padding-top:1rem;border-top:1px solid rgba(74,44,23,0.1);">
                    ...
                    <div style="font-size:.85rem;font-weight:500;color:var(--kafe-brown);">{{ auth()->user()->created_at->format('F Y') }}</div>
                </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input class="form-control-kafe @error('first_name') is-invalid @enderror"
                                   ...
                                   required>
                            @error('first_name')
                                <div style="font-size:.73rem;color:#c0392b;">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
