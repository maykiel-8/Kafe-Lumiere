...
<nav>
    ...
    <div class="nav-links">
        <a href="{{ route('admin.profile.show') }}"><i class="bi bi-person-circle"></i> Profile</a>
...
<div class="main">
    ...
    {{-- Flash --}}
    @if(session('success'))
        <div class="alert-s"><i class="bi bi-check-circle"></i> {{ session('success') }}</div>
    @endif
    @if($errors->any())
        <div class="alert-e"><i class="bi bi-exclamation-circle"></i> {{ $errors->first() }}</div>
    @endif
    ...
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:1.8rem;">
        ...
        <div>
            ...
            <div style="font-size:.78rem;color:#888;">{{ auth()->user()->email }}</div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-4">
            <div class="stat-card">
                ...
                <div class="stat-value">{{ $myReviews->count() }}</div>
            </div>
        </div>
        <div class="col-4">
            <div class="stat-card">
                <div class="stat-label">To Review</div>
                <div class="stat-value">{{ $pendingReviews->count() }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="tab-group">
        ...
        <button class="tab-btn" onclick="switchTab('reviews')">
            ...
            @if($pendingReviews->count()) <span style="background:var(--kafe-caramel);color:#fff;border-radius:50%;width:18px;height:18px;font-size:.65rem;display:inline-flex;align-items:center;justify-content:center;margin-left:4px;">{{ $pendingReviews->count() }}</span> @endif
        </button>
    </div>
            ...
            <div class="col-lg-8">
                ...
                <div style="display:flex;gap:7px;flex-wrap:wrap;margin-bottom:1rem;align-items:center;">
                    ...
                    @foreach($categories as $cat)
                        <button class="btn-outline" style="font-size:.75rem;padding:4px 12px;" onclick="filterCat({{ $cat->id }},this)">{{ $cat->name }}</button>
                    @endforeach
                ...
                {{-- Price range slider (matches home page style) --}}
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:1rem;flex-wrap:wrap;">
                    <span style="font-size:.75rem;color:#888;">Max Price:</span>
                    <input type="range" id="priceRange" min="0" max="{{ $maxProductPrice }}" value="{{ $maxProductPrice }}" step="10"
                    ...
                    <span id="priceVal" style="font-size:.78rem;font-weight:500;color:var(--kafe-brown);min-width:42px;">₱{{ $maxProductPrice }}</span>
                    <button onclick="document.getElementById('priceRange').value={{ $maxProductPrice }};document.getElementById('priceVal').textContent='₱{{ $maxProductPrice }}';renderProducts();"
                ...
                <div class="row g-2" id="productGrid">
                    ...
                    <div class="col-6 col-md-4 product-row"
                        ...
                        <div class="product-tile"
                            ...
                            @if($tile_photo)
                                <img src="{{ asset('storage/'.$tile_photo) }}"
                                     alt="{{ $p->name }}"
                            ...
                            <div style="padding:.5rem .6rem;">
                                ...
                                <div class="p-price">₱{{ number_format($p->price_tall,2) }}</div>
                                @if($p->addons->count())
                                    <div style="font-size:.68rem;color:#888;margin-top:2px;">
                                        {{ $p->addons->pluck('name')->join(', ') }}
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                <div class="kafe-card" style="position:sticky;top:76px;">
                    ...
                    <div class="kafe-card-body" style="padding:1rem;">
                        ...
                        <div id="cartTotals" style="display:none;border-top:1px solid rgba(74,44,23,.1);padding-top:.75rem;margin-top:.5rem;">
                            ...
                            {{-- Cash only for customer walk-up orders --}}
        ...
        <div class="kafe-card">
            ...
            @else
                <table class="table mb-0" style="width:100%;">
                    ...
                    <tr>
                        <td><strong style="font-family:var(--font-serif);">{{ $order->order_number }}</strong></td>
                        <td>
                            <div style="font-size:.78rem;color:#888;">
                                ...
                                @if($order->orderItems->count() > 2)
                                    <span style="color:var(--kafe-caramel);">+{{ $order->orderItems->count()-2 }} more</span>
                                @endif
                            </div>
                        </td>
                        <td><strong>₱{{ number_format($order->total,2) }}</strong></td>
                        <td style="font-size:.78rem;text-transform:uppercase;">{{ $order->payment_method }}</td>
                        <td><span class="badge-{{ $order->status }}">{{ ucfirst($order->status) }}</span></td>
                        <td style="font-size:.78rem;color:#888;">{{ $order->created_at->format('M d, Y H:i') }}</td>
                    </tr>
                    @endforeach
                    </tbody>
                </table>
                <div style="padding:1rem 1.3rem;">{{ $orders->links() }}</div>
            @endif
        </div>
    </div>
    ...
    <div class="tab-pane" id="tab-reviews">
        <div class="row g-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-ctrl" name="product_id" required onchange="setTxnId(this)">
                                        ...
                                        @foreach($pendingReviews as $item)
                                            <option value="{{ $item['product_id'] }}" data-txn="{{ $item['transaction_id'] }}">
                                                {{ $item['product_name'] }} — {{ $item['order_number'] }} ({{ $item['date'] }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            <div class="col-lg-6">
                <div class="kafe-card">
                    <div class="kafe-card-header">My Reviews ({{ $myReviews->count() }})</div>
                        ...
                        <div style="padding:.9rem 1.2rem;border-bottom:1px solid rgba(74,44,23,.07);">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:3px;">
                                ...
                                <div style="display:flex;align-items:center;gap:8px;">
                                    ...
                                    <button onclick="toggleEditReview({{ $rev->id }})"
                            ...
                            <div style="font-size:.8rem;color:#666;">{{ $rev->comment }}</div>
                            <div style="font-size:.72rem;color:#aaa;margin-top:2px;">{{ $rev->created_at->format('M d, Y') }}</div>

                            {{-- Inline edit form (hidden by default) --}}
                            <div id="editReview_{{ $rev->id }}" style="display:none;margin-top:.75rem;background:var(--kafe-pearl);border-radius:8px;padding:.85rem;">
                                <form action="{{ route('reviews.update', $rev) }}" method="POST">
                                    @csrf @method('PUT')
                                    <div style="margin-bottom:.5rem;">
                                        <label style="font-size:.75rem;font-weight:500;color:var(--kafe-brown);display:block;margin-bottom:3px;">Rating</label>
                                        <div class="star-picker-edit" data-review="{{ $rev->id }}" style="font-size:1.4rem;cursor:pointer;color:var(--kafe-gold);">
                                            @for($s=1;$s<=5;$s++)
                                                <span data-val="{{ $s }}">{{ $s <= $rev->rating ? '★' : '☆' }}</span>
                                            @endfor
                                        </div>
                                        <input type="hidden" name="rating" id="editRating_{{ $rev->id }}" value="{{ $rev->rating }}">
                                    </div>
                                    <div style="margin-bottom:.5rem;">
...
function placeOrder() {
    ...
    .then(res => {
        ...
        let mailNote = '';
        if (res.mail_sent) {
            mailNote = `<div style="font-size:.75rem;color:#3d5c35;margin-top:.75rem;text-align:center;background:rgba(122,140,110,.1);padding:.5rem;border-radius:8px;"><i class="bi bi-envelope-check"></i> Receipt emailed to <strong>{{ auth()->user()->email }}</strong></div>`;
