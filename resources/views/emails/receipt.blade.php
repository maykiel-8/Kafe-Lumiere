...
<div class="container">
  ...
  {{-- Greeting --}}
  <div class="greeting">
    Dear <strong>{{ $transaction->customer_name }}</strong>,<br>
  ...
  <div class="info-block">
    <div class="info-row">
      ...
      <span class="info-value">{{ $transaction->order_number }}</span>
    </div>
    <div class="info-row">
      <span class="info-label">Date &amp; Time</span>
      <span class="info-value">{{ $transaction->created_at->timezone('Asia/Manila')->format('M d, Y h:i A') }}</span>
    </div>
    <div class="info-row">
    ...
    <div class="info-row">
      ...
      <span class="info-value">{{ ($transaction->cashier && $transaction->cashier->role === 'customer') ? 'Kafé Lumière' : ($transaction->cashier->full_name ?? 'Kafé Lumière') }}</span>
    </div>
    ...
    <div class="item-row">
      <div class="item-left">
        ...
        @if($item->addons->count())
          <div class="item-addon">+ {{ $item->addons->pluck('addon_name')->join(', ') }}</div>
        @endif
      </div>
  ...
  <div class="totals">
    <div class="total-row">
      ...
      <span class="total-val">&#8369;{{ number_format($transaction->subtotal, 2) }}</span>
    </div>
    <div class="total-row">
      <span class="total-label">VAT (12%)</span>
      <span class="total-val">&#8369;{{ number_format($transaction->tax, 2) }}</span>
    </div>
    @if($transaction->amount_tendered)
    <div class="total-row">
      <span class="total-label">Amount Tendered</span>
      <span class="total-val">&#8369;{{ number_format($transaction->amount_tendered, 2) }}</span>
    </div>
    <div class="total-row">
      <span class="total-label">Change</span>
      <span class="total-val">&#8369;{{ number_format($transaction->change, 2) }}</span>
    </div>
    @endif
  </div>
  ...
  <div class="grand-total">
    ...
    <span class="grand-val">&#8369;{{ number_format($transaction->total, 2) }}</span>
  </div>
